...
DL_mixture_function <- function(y, d_lamb = 2, nrun_lamb = 10^4, burn_lamb = 10^3,  thin_lamb = 2,
  ...
  result.lamb <- DL_mixture(conc_dir = conc_dir, diag_psi_iw=diag_psi_iw, niw_kap=niw_kap, niw_nu=nu,
                          as=a_s, bs=b_s, a=a,
                          nrun=nrun_lamb, burn=burn_lamb, thin=thin_lamb,
                          nstep = 5, prob = 0.5,
                          Lambda = lambda, eta = eta, Y = y,
                          del = cluster.start,
                          dofactor=1 )
  ...
  #print(paste0("nclus_chain_R: ", apply(post.samples, 1, function(x) length(unique(x))) ))
