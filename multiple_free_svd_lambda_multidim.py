...
for j in range(n_reruns):
  ...
  V = ortho_group.rvs(dim = p, random_state = seed)
  V = V[0:p,0:d]
  ...
  for rho in rho_s:
      ...
      list_performance.append(["APPLAM", p,dtrue,d,M,npc,sampler_aniso.means_ar, sampler_aniso.lambda_ar, rho, post_mode_nclus_aniso,
                          post_avg_nclus_aniso, post_avg_nonall_aniso, ri_best_clus_aniso, CI_ris_aniso, ari_best_clus_aniso, CI_aris_aniso,
                          ranges[0][0]])

      list_performance.append(["Isotropic", p,dtrue,d,M,npc,sampler_iso.means_ar, sampler_iso.lambda_ar, rho, post_mode_nclus_iso,
                          post_avg_nclus_iso, post_avg_nonall_iso, ri_best_clus_iso, CI_ris_iso, ari_best_clus_iso, CI_aris_iso, ranges[0][0]])
  
  

  
df_performance = pd.DataFrame(list_performance, columns=('model', 'p','dtrue','d','M','npc','means_ar','lambda_ar', 'intensity',
                                    'mode_nclus', 'avg_nclus', 'avg_nonalloc', 'ri_best_clus', 'CI_ris', 'ari_best_clus', 'CI_aris', 'ranges'))
