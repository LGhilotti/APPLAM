...
def generate_data_student(Lambda, etas, sigma_bar_cov, seed):
    means = np.matmul(Lambda,etas.T)
    sigma_bar_cov_mat = np.diag(sigma_bar_cov)
    out = np.vstack([multivariate_t_rvs(m = means[:,i], S = sigma_bar_cov_mat, df = 5, seed = seed)[0] for i in range(etas.shape[0])])
    return out
...
DEFAULT_PARAMS_FILE_ANISO = "data/Fixed_lambda/resources/sampler_params_aniso.asciipb"
DEFAULT_PARAMS_FILE_ISO = "data/Fixed_lambda/resources/sampler_params_iso.asciipb"
...
sigma_bar_prec = np.repeat(10000, p)
...
seed = 1222432
np.random.seed(seed)
etas_1 = np.matrix(np.random.normal(loc = -2, size = npc).reshape(npc,1))
etas_2 = np.matrix(np.random.normal(loc = 2, size = npc).reshape(npc,1))
etas = np.concatenate((etas_1,etas_2))
print(etas)
data = generate_data_student(lamb, etas, sigma_bar_cov, seed)          


np.savetxt('data/Fixed_lambda/applam/dataset.csv', data, delimiter=',', fmt='%f')
...
for d in d_s:
  ...
  outpath_d = "data/Fixed_lambda/applam/comp_app_p_{0}_d_{1}_dtrue_{2}_M_{3}_npc_{4}_out".format(p,d,dtrue,M,npc)
  ...
  for rho in rho_s:
      ...
      for j in range(n_reruns):
        ...
        # Run the algorithm
        sampler_aniso.run(ntrick, nburn, niter, thin, data_scaled, d, 2*lamb, ranges, fix_lambda = "TRUE", fix_sigma = "TRUE", log_every = log_ev)
        ...
        # Run the algorithm
        sampler_iso.run(ntrick, nburn, niter, thin, data_scaled, d, 2*lamb, ranges,  fix_lambda = "TRUE", fix_sigma = "TRUE", log_every = log_ev)
